	...
	<!-- contents -->
			...
			<div id="contents">
				...
				<div id="customer">
					...
					<form action="admin_faq_update_ok" name="notice" id="notice" method="post">
						...
						<table class="checkTable" border="1" cellspacing="0">
							...
							<tbody>
									...
									<tr>
										...
										<option value="product" <c:if test="${fdto.f_type eq 'product'}"> selected="selected" </c:if>>상품</option>
										<option value="order" <c:if test="${fdto.f_type eq 'order'}"> selected="selected" </c:if>>주문</option>
										<option value="join" <c:if test="${fdto.f_type eq 'join'}"> selected="selected" </c:if>>회원가입</option>
									...
									<td>
										...
										<input type="hidden" class="wlong" name="f_num" value="${fdto.f_num}">
									</td>
