					...
					<c:choose>
						<c:when test="${searchFlag != null }">
							<div class="btnAreaList">
								<div class="allPageMoving1">
									<!-- 첫 페이지 이동 -->
									<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${startpage }" class="n">
									...
									<c:if test="${page > 1 }">
										<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${page - 1 }" class="pre">
											<img src="../images/btn/btn_pre1.gif" alt="앞페이지로"/>
										</a>
									</c:if>
									<!-- 순차적으로 페이지 출력 -->
							   	 	<c:forEach var="a" begin="${startpage }" end="${endpage }" step="1">
								   	 	<span class="page_num">
									   	 	<c:choose>
										   	 	...
										   	 	<c:when test="${a != page }">
											   	 	<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${a}">${a}</a>
										   	 	</c:when>	
									   	 	</c:choose>			   	 		   	 	
								   	 	</span>
							   	 	</c:forEach>
									...
									<c:if test="${page < maxpage }">
										<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${page + 1 }" class="next">
											<img src="../images/btn/btn_next1.gif" alt="뒤페이지로"/>
										</a>
									</c:if>
									<!-- 마지막 페이지 이동 -->
									<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${maxpage }" class="n">
						...
						<!-- searchFlag가 없을때 -->
							<div class="btnAreaList">
								<div class="allPageMoving1">
									<!-- 첫 페이지 이동 -->
									<a href="prizewinner_list?page=${startpage }" class="n">
										<img src="../images/btn/btn_pre2.gif" alt="처음으로"/>
									</a>
									<!-- 이전 페이지 이동버튼 -->
									<c:if test="${page <= 1 }">
									...
									<!-- 마지막 페이지 이동 -->
									<a href="prizewinner_list?page=${maxpage }" class="n">
										<img src="../images/btn/btn_next2.gif" alt="마지막페이지로"/>
									</a>
								</div>
							</div>
...
<!-- 			   	 	첫 페이지 이동 -->
<%-- 			   	 	<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${startpage }"> --%>
<!-- 			   			<span class="page_num"> -->
<!-- 			   	 		<strong>◁◁</strong> -->
<!-- 			   	 		</span> -->
<!-- 			   	 	</a> -->
...
<%-- 			   	 	<c:if test="${page > 1 }"> --%>
<!-- 			   			<span class="page_num"> -->
<%-- 			   	 		<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${page - 1 }"><strong>◁</strong></a> --%>
<!-- 			   	 		</span> -->
<%-- 			   	 	</c:if> --%>
...
<%-- 					   	 	</c:when>	 --%>
<%-- 					   	 	<c:when test="${a != page }"> --%>
<%-- 						   	 	<a href="prizewinner_list?searchFlag=${searchFlag}&opt=${opt}&search=${search}&page=${a}"><strong>${a}</strong></a> --%>
<%-- 					   	 	</c:when>	 --%>
<%-- 				   	 	</c:choose>			   	 		   	 	 --%>
<!-- 				   	 	</span> -->
<%-- 			   	 	</c:forEach> --%>
...
<!-- 			   	 	마지막 페이지 이동 -->
<%-- 			   	 	<a href="prizewinner_list?page=${maxpage }"> --%>
<!-- 			   			<span class="page_num"> -->
<!-- 			   	 		<strong>▷▷</strong> -->
<!-- 			   	 		</span> -->
<!-- 			   	 	</a> -->
...
<!-- 					//페이징이동1 끝 -->
					...
					<div class="searchWrap">
						<form action="prizewinner_list" method="get">
						<div class="search">
							<ul>
								...
								<li class="web"><input type="submit" src="../images/btn/btn_search.gif" alt="검색" /></li>
								<li class="mobile"><input type="submit" src="../images/btn/btn_search_m.gif" alt="검색" /></li>
							</ul>
						</div>
						</form>
					</div>
