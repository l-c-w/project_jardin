...
<script type="text/javascript">
	...
	function p_Fnq_updateForm(pf_num, pf_content) {
		...
		html += "</td>";
		html += "<td><a href='#' onclick='p_Fng_update()'>완료</a>";
		html += "&nbsp&nbsp&nbsp&nbsp&nbsp";
		html += "<a href='#' onclick='p_Fng_cancel()'>취소</a>";
		html += "</td></tr>";
					...
					success : function(data) { // data에 값이 담김
						...
						if (data.length > 0) {
							for (var i = 0; i < data.length; i++) {
								html += "<tr><td colspan='2'><h5>제목 :"
										+ data[i].pf_title + " </h5></td></tr>";
								html += "<tr id='updateForm"+data[i].pf_num+"'><td>내용 :"
										+ data[i].pf_content + "</td>";
								html += `<td><a href=# onclick="p_Fnq_updateForm('\${data[i].pf_num}','\${data[i].pf_content}')">수정</a>`;
								html += "&nbsp&nbsp&nbsp&nbsp&nbsp";
								html += "<a href='#' onclick='p_Fnq_delete("
										+ data[i].pf_num + ")'>삭제</a>";
								html += "</td></tr>";
							}
						} else {
	...
	<div id="layerWrap">
		...
		<div class="inputWrap">
			...
			<div class="inputBody">
				...
				<p class="close">
					<a onclick="parent.$.fancybox.close();" href="javascript:;"><img
						src="../images/btn/btn_input_close.gif" alt="닫기" /></a>
				</p>
					...
					<table>
						...
						<tr>
							<td><textarea rows="4" cols="110" class="re" id="pf_content"
								...
								type="button" onclick="p_Fng_insert()" value="질문 등록"><br>
							</td>
						</tr>
					</table>
				</form>
